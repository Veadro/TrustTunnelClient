...
void ffthd_sleep(uint ms)
{
	...
	while (0 != nanosleep(&ts, &ts) && errno == FF_EINTR) {
	}
}
...
static int _ffps_perf(int who, struct ffps_perf *p, uint flags)
{
	...
	if (flags & FFPS_PERF_CPUTIME) {
		...
		struct rusage u;
		if (0 == (r = getrusage(who, &u))) {
			timeradd(&u.ru_utime, &u.ru_stime, &u.ru_utime);
			fftime_fromtimeval(&p->cputime, &u.ru_utime);
		}
#endif
		rc |= r;
	}
