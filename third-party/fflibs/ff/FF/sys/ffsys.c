...
int fffile_writeall(const char *fn, const ffstr *data, uint flags)
{
	int rc = 1;
	fffd f = FF_BADFD;
	if (FF_BADFD == (f = fffile_open(fn, O_CREAT | O_WRONLY | flags)))
		goto done;

	if (data->len != (size_t)fffile_write(f, data->ptr, data->len))
		goto done;
